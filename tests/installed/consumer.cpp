...
int main()
{
  ...
  auto request = pkgresolve::resolution_request::seal(
      ...
      {pkgresolve::resolution_goal(
          ...
          "installed-consumer")});
  const auto result = pkgresolve::resolve(std::move(request));
  if (result.selections().size() != 1 || result.goals().size() != 1)
    return 1;
  ...
  if (selected == nullptr || selected->candidate() == nullptr)
    return 2;
  if (selected->release().package().name() != "installed-consumer")
    return 3;
  return 0;
}
