...
for expected in libpkgsource.so.3 libpkgcatalog.so.3 libpkgstate.so.4; do
  printf '%s\n' "$needed" | grep -Fx "$expected" >/dev/null ||
    fail "missing direct NEEDED $expected"
done
for obsolete in libpkgsource.so.1 libpkgsource.so.2 libpkgcatalog.so.1 libpkgcatalog.so.2 libpkgstate.so.1 libpkgstate.so.2 libpkgstate.so.3; do
