...
for variable in LIBPKGSOURCE_SOURCE LIBPKGCATALOG_SOURCE LIBPKGSTATE_SOURCE; do
  eval "value=\${$variable:-}"
...
consumer_flags=$(PKG_CONFIG_PATH="$consumer_pkgconfig" pkg-config --cflags libpkgresolve)
if [ "$link_mode" = static ]; then
  consumer_libs=$(PKG_CONFIG_PATH="$consumer_pkgconfig" pkg-config --static --libs libpkgresolve)
else
  consumer_libs=$(PKG_CONFIG_PATH="$consumer_pkgconfig" pkg-config --libs libpkgresolve)
fi
